...
const PROTOCOL_CHECKS = [
  {
    id: "tag",
    label: "Sello tag",
    detail: "Demo article publishes machine-readable license terms.",
  },
  {
    id: "llms",
    label: "llms.txt",
    detail: "Crawler policy is available in /llms.txt.",
  },
  {
    id: "tdm",
    label: "tdm-policy",
    detail: "TDM policy is available in /tdm-policy.json.",
  },
  {
    id: "onchain",
    label: "Solana PDA",
    detail: "Content record points to the devnet Sello program.",
  },
];
      ...
      <div className="grid gap-8 md:grid-12 lg:grid-cols-3">
        ...
        <section className="lg:col-span-2 space-y-6">
          ...
          <div className="space-y-4">
            {articles.length === 0 ? (
              <div className="postal-card p-12 sm:p-20 text-center italic text-muted">
                No articles found in this session.
              </div>
            ) : (
              articles.map((art, idx) => (
                <div
                  key={idx}
                  className="postal-card p-6 sm:p-8 border-primary/20 hover:bg-card/50 transition-colors"
                >
                  <div className="flex flex-col items-start justify-between gap-6 sm:flex-row">
                    <div className="space-y-3 w-full">
                      <p className="max-w-full break-all text-base leading-tight text-cream sm:max-w-md sm:text-lg sm:break-normal sm:truncate">
                        {art.url}
                      </p>
        ...
        <section className="space-y-8">
          <h3 className="font-display text-2xl sm:text-3xl uppercase tracking-widest text-gold text-center">
            Protocol Status
          </h3>
          <div className="postal-card overflow-hidden">
            <div className="bg-gold/10 p-4 text-center border-b border-border-low">
              <p className="font-mono text-xs uppercase text-gold tracking-widest font-black">
                Current Deployment
              </p>
            </div>
            <div className="divide-y divide-border-low">
              {PROTOCOL_CHECKS.map((item) => (
                <div
                  key={item.id}
                  className="p-4 sm:p-5 flex items-center justify-between gap-4 group hover:bg-gold/5 transition-colors"
                >
